...
## Script to read in bsp hysplit output, AB obs & BC obs and make it into daily accumulated
...
from utils.funcs import wdcompliant, get_title, plot_day
...
# date = str(sys.argv[1]) #forecast date of interest - will define input file
# len = str(sys.argv[2])
date = 20190526
dur = 7
...
for i in range(dur):
    ...
    ## Read in bsp ouput files of interest --> obs funcs read in data inside
    ...
    nc = wdcompliant(filein)                           #bsp output
    ab_obs_in = get_ab_obs(ab_obs_file, f"{date}00")   #ab obs, see ab_pm.py in utils
    bc_obs_in = get_bc_obs(date)                       #bc obs, see bc_pm.py in utils
    print(nc.variables["time"][:])

    ## Resample data to get daily sums
    pm_daily, ab_daily, bc_daily = np.squeeze(nc.variables["pm25"][:])[0,:,:], 0, 0
    for i in range(22):
        pm_daily = np.sum([pm_daily,np.squeeze(nc.variables["pm25"][:])[i+1,:,:]])
        ab_daily = ab_daily + np.squeeze(ab_obs_in.variables["pm25"][i])
        bc_daily = bc_daily + np.squeeze(bc_obs_in.variables["pm25"][i])
    
    ncpm.append(pm_daily)
    abpm.append(ab_daily)
    bcpm.append(bc_daily)                   
    print(size(ncpm))
    print(ncpm)
    date = datetime.datetime.strptime(str(date),"%Y%m%d") + timedelta(days=1)
    ...
    im = plot_day(ncpm,i,date, plot_extent,fig,lons,lats) ## plots model data and formats the map all pretty with borders etc. 
    plt.scatter(obslons,obslats, c= abpm,marker = 's', alpha=1, s=100,zorder=2,cmap='gist_heat_r',edgecolors='black',vmin=0,vmax=var.max()) ## plots ab observational data
    plt.scatter(bclons,bclats, c= bcpm,marker = 'o', alpha=1, s=100,zorder=2,cmap='gist_heat_r',edgecolors='black',vmin=0,vmax=var.max()) ## plots bc observational data)
...
plt.savefig(                                                            ## save plot as .png named after date and case study 
    str(img_dir)+ f"/wd-daily-{date}-{scheme_name}.png", bbox_inches="tight"
)